...
const stacklist: { language: string; stack: string[] }[] = [
  {
    language: "JavaScript",
...
const Tooltip = ({ text, position, stacks }: params) => {
...
export default function Languages() {
  ...
  const handleMouseEnter = (item: string, event: any) => {
    const data =
      stacklist.find((stack) => stack.language === item)?.stack || [];
    setStack(data);
    const rect = event.currentTarget.getBoundingClientRect();
    setTooltip({
      show: true,
      text: item,
      position: { top: rect.top + rect.height + 10, left: rect.left },
    });
  };
  ...
  return (
    <div
      className={cn(
        "relative z-20 max-w-xl w-full mx-auto overflow-hidden mb-7",
        "[mask-image:linear-gradient(to_right,transparent,white_10%,white_90%,transparent)]"
      )}
    >
      <ul
        className={cn(
          "flex flex-wrap gap-4 md:gap-6 justify-center py-4",
          "animate-scroll",
          "hover:[animation-play-state:paused]"
        )}
      >
        <blockquote
          onMouseEnter={(event) => handleMouseEnter("JavaScript", event)}
          onMouseLeave={handleMouseLeave}
          className="flex items-center gap-1 justify-center select-none font-medium transition-transform hover:scale-110 text-sm text-zinc-800 dark:text-zinc-100"
        >
          <svg
        ...
        <blockquote
          onMouseEnter={(event) => handleMouseEnter("TypeScript", event)}
          onMouseLeave={handleMouseLeave}
          className="flex items-center gap-1 justify-center select-none font-medium transition-transform hover:scale-110 text-sm text-zinc-800 dark:text-zinc-100"
        >
          <svg
        ...
        <blockquote
          onMouseEnter={(event) => handleMouseEnter("Python", event)}
          onMouseLeave={handleMouseLeave}
          className="flex items-center gap-1 justify-center select-none font-medium transition-transform hover:scale-110 text-sm text-zinc-800 dark:text-zinc-100"
        >
          <svg
        ...
        <blockquote
          onMouseEnter={(event) => handleMouseEnter("Go", event)}
          onMouseLeave={handleMouseLeave}
          className="flex justify-center select-none font-medium transition-transform hover:scale-110 text-sm text-zinc-800 dark:text-zinc-100"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            className="w-7 h-7"
        ...
        <blockquote
          onMouseEnter={(event) => handleMouseEnter("SQL", event)}
          onMouseLeave={handleMouseLeave}
          className="flex items-center gap-1 justify-center select-none font-medium transition-transform hover:scale-110 text-sm text-zinc-800 dark:text-zinc-100"
        >
          <svg
          ...
          >
            <path
              fill="currentColor"
              d="M12 9.828c4.17 0 7.549-1.528 7.549-3.414C19.549 4.53 16.169 3 12 3S4.452 4.529 4.452 6.414S7.832 9.828 12 9.828"
            />
            <path
              fill="currentColor"
        ...
        <blockquote
          onMouseEnter={(event) => handleMouseEnter("Tailwind CSS", event)}
          onMouseLeave={handleMouseLeave}
          className="flex items-center gap-1 justify-center select-none font-medium transition-transform duration-300 hover:scale-110 text-sm text-zinc-800 dark:text-zinc-100"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            className="w-5 h-5"
